...
SELECT
	...
	category.name,
	MAX(film.rental_rate),
	MAX(film.rental_rate) OVER(ORDER BY category.category_id ASC ROWS BETWEEN 3 PRECEDING AND 3 FOLLOWING) AS rolling_max,
	MIN(film.rental_rate),
	MIN(film.rental_rate) OVER(ORDER BY category.category_id ASC ROWS BETWEEN 3 PRECEDING AND 3 FOLLOWING) AS rolling_min
FROM category
...
SELECT
	...
	category.name,
	MAX(film.rental_rate),
	MAX(film.rental_rate) OVER(PARTITION BY category.category_id ROWS BETWEEN 3 PRECEDING AND 3 FOLLOWING) AS rolling_max,
	MIN(film.rental_rate),
	MIN(film.rental_rate) OVER(PARTITION BY category.category_id ROWS BETWEEN 3 PRECEDING AND 3 FOLLOWING) AS rolling_min
FROM category
